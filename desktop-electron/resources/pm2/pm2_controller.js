#!/usr/bin/env node
/**
 * Prism 桌面端 PM2 Controller —— 替代原 pm2.py。
 *
 * 职责：
 ...
 *  2. 提供与原 pm2 完全一致的 HTTP API 契约（:7002 → GET /api/status|health|diagnostics|
 *     restart-status；POST /api/start|stop|restart|restart/<name>），使 Electron 主进程、preload、
 *     renderer 及后端 /api/v1/system/pm2/* 代理无需改动。
 *  3. 把发现信息写入状态文件（PRISM_PM2_STATE_PATH 或默认路径），供主进程连接。
 *
...
// ── pm2 上报/控制的“服务型”进程（redis/前端也交由 PM2 托管）──
const MANAGED_PM2_SERVICES = ['prism-backend', 'prism-worker', 'prism-celery', 'hermes-dashboard', 'hermes-webui', 'deepseek-harness', 'prism-frontend', 'prism-redis'];
...
const statePath =
  process.env.PRISM_PM2_STATE_PATH ||
  path.join(os.homedir(), '.prism-pm2', 'state.json');

// ── API 端口：默认 7002，被占用时回退到可用端口并写入状态文件 ──
const preferredApiPort = Number.parseInt(process.env.PM2_API_PORT || process.env.PRISM_PM2_PORT || '7002', 10);
...
async function serviceStatus(name) {
  ...
  } else if (name === 'prism-frontend') {
    base.port = servicePorts['prism-frontend'] || 3000;
    base.url = `http://127.0.0.1:${base.port}`;
    base.frontend_url = base.url;
  } else if (name === 'redis') {
    base.port = servicePorts['redis'] || 6379;
    base.url = `http://127.0.0.1:${base.port}`;
    base.redis_url = base.url;
  }
  return base;
}
...
async function startAll() {
  ...
  // 空 daemon 时 delete('all') 可能报“无进程可删”，属正常，忽略之。
  try {
  ...
  for (const app of eco.apps) {
    ...
    if (app.name === 'deepseek-harness') {
      // 与原 pm2 一致：deepseek-harness 由外部（DSH harness / 本套件）托管于规范端 :3080，
...
function restartService(service) {
  // 允许直接传 PM2 进程名（如 prism-frontend），也接受 pm2 服务名（如 backend）。
...
async function handle(req, res) {
  ...
  if (method === 'GET' && p === '/api/health') {
    return sendJson(res, { status: 'ok', message: 'PM2 is running', pid: CONTROLLER_PID, launchToken: LAUNCH_TOKEN, apiPort });
  }
...
async function main() {
  const autoStart = process.env.PRISM_PM2_DISABLE_START !== '1';
