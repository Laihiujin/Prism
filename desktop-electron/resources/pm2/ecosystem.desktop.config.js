...
// 替代原先的 pm2.py。路径均相对本文件目录（__dirname）解析，
// 因此在打包的 Resources 目录或源码 desktop-electron/resources 下都能直接运行。
const path = require('path');
const fs = require('fs');
const { spawnSync } = require('child_process');

const CONTROLLER_DIR = __dirname;                 // .../Resources/pm2
...
// 记录端口：与 pm2 默认一致，后端/worker/dashboard/webui 由 Electron 主进程经
...
function webuiPyLaunch() {
  // 与 pm2 的 _build_run_path_launch 等价：runpy 运行 hermes-webui/server.py，
...
const apps = [
  ...
  },
  {
    name: 'prism-celery',
    script: PY,
    args: ['-m', 'celery', '-A', 'fastapi_app.tasks.celery_app', 'worker', '-l', 'info', '-P', 'threads', '--concurrency=4'],
    cwd: BACKEND,
    exec_interpreter: 'none',
    autorestart: true,
    restart_delay: 2000,
...
module.exports = {
  apps,
  // 供 pm2_controller 使用的服务元数据：pm2 服务名 -> PM2 进程名
  _prism: {
    ...
    // pm2 的 /api/status 用的 service key（连字符转下划线）
    serviceKeys: ['backend', 'automation-worker', 'celery-worker', 'hermes-gateway', 'hermes-dashboard', 'hermes-webui', 'prism-frontend', 'redis'],
  },
};
