...
def get_repo_root() -> Path:
    return _resolve_env_path("PRISM_RESOURCES_PATH", "PRISM_APP_ROOT") or get_source_repo_root()
...
def get_prismenv_root() -> Path:
    raw_override = (os.getenv("PRISM_HERMES_PYTHON") or "").strip()
    override = Path(raw_override).expanduser().absolute() if raw_override else None
    if override is not None and override.exists():
        # Do not resolve the executable symlink: venv/bin/python commonly points
        # at the base interpreter, but its parent venv owns the runtime marker
    ...
    candidates = [
        get_repo_root() / "prismenv",
        get_source_repo_root() / "prismenv",
    ]
...
def get_workspace_root() -> Path:
    workspace_root = _resolve_env_path("PRISM_HERMES_WORKSPACE")
...
def get_hermes_home_path() -> Path:
    return _resolve_env_path("PRISM_HERMES_HOME") or (get_source_repo_root() / "tools" / "hermes-home")
...
def get_hermes_webui_state_path() -> Path:
    return _resolve_env_path("PRISM_HERMES_WEBUI_STATE_DIR") or (get_hermes_home_path() / "webui")


def get_hermes_python_path() -> Path:
    prismenv_root = get_prismenv_root()
    candidates = [
        prismenv_root / "Scripts" / "python.exe",
        prismenv_root / "bin" / "python",
    ]
    for candidate in candidates:
        if candidate.exists():
            return candidate
...
def get_hermes_runtime_stamp_path() -> Path:
    return get_prismenv_root() / ".hermes-runtime-ready"
