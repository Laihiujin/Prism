"""Project-local Hermes CLI bridge for the Prism agent UI."""
...
def _build_project_capabilities_block() -> str:
    ...
    from .publishing_skills import render_publishing_skills
    ...
    lines = [
        ...
        "- Project publishing bootstrap: docs/agent-bootstrap.md",
    ...
    lines.append(render_publishing_skills())
...
def _build_prompt(goal: str, context: Optional[Dict[str, Any]]) -> str:
    sections = [
        "You are Hermes Agent running inside the Prism project.",
...
def _build_process_env(home_path: Path) -> Dict[str, str]:
    ...
    env["PRISM_WORKSPACE_ROOT"] = str(get_workspace_root())
...
def _dashboard_url(port: int) -> str:
    public_host = str(os.getenv("PRISM_HERMES_PUBLIC_HOST") or "127.0.0.1").strip() or "127.0.0.1"
    return f"http://{public_host}:{port}"


def _get_runtime_bind_host() -> str:
    bind_host = str(os.getenv("PRISM_HERMES_BIND_HOST") or "127.0.0.1").strip()
...
async def get_hermes_runtime_status(
    ...
    port = port if isinstance(port, int) and port > 0 else _get_runtime_port("PRISM_HERMES_DASHBOARD_PORT", 9119)
    webui_port = (
        webui_port
...
async def start_hermes_dashboard(
    ...
    resolved_port = (
        ...
        else _get_runtime_port("PRISM_HERMES_DASHBOARD_PORT", 9119)
    )
