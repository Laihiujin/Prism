...
def _default_browsers_dir() -> Path:
    ...
    prism_backend_dir = Path(__file__).resolve().parents[1]
    repo_root = prism_backend_dir.parent
...
def ensure_playwright_chromium_installed(
    *,
    ...
    # Development/runtime fallback: reuse an installed system Chrome. Patchright can
    # drive Chrome directly and does not require its own Chromium download.
    try:
        from utils.chrome_detector import get_chrome_executable

        detected_chrome = Path(get_chrome_executable())
        if detected_chrome.is_file():
            os.environ.setdefault("LOCAL_CHROME_PATH", str(detected_chrome))
            return PlaywrightBootstrapResult(
                browsers_path=os.environ["PLAYWRIGHT_BROWSERS_PATH"],
                chromium_executable=str(detected_chrome),
                installed=True,
                install_ran=False,
                error=None,
            )
