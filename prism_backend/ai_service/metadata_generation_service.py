"""Reusable AI metadata (title/desc/tags) generation for files.
...
parsing and persistence for one or more file_ids.
"""
...
async def _generate_one(
    ...
    """Generate + persist metadata for a single file. Returns a result dict."""
    cursor.execute("""
    ...
    # 平台网感 prompt（注入平台风格 + 字数/话题上限 + 可选语言）
    ...
    # 按平台限制做后处理：标题截断到平台上限、话题数上限与去重
    from ai_service.title_topic_generator import apply_platform_limits

    ai_title, ai_tags, ai_description = apply_platform_limits(
        platform=platform,
        title=ai_title,
        tags=ai_tags,
    ...
    if logger:
        logger.info("AI metadata generated for file %s: %s", file_id, ai_title)
...
async def generate_metadata_for_files(
    ...
    """Generate + persist AI metadata for a list of file_ids.
    ...
    for file_id in file_ids:
        try:
            ...
            if logger:
                logger.error("Failed to generate AI metadata for file %s: %s", file_id, exc)
