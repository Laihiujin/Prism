...
Playwright扫码实现
复制自: prism_backend/fastapi_app/api/v1/auth/services.py::XiaohongshuLoginService
"""
import asyncio
import uuid
...
class XiaohongshuAdapter(PlatformAdapter):
    ...
    async def poll_status(self, session_id: str) -> LoginResult:
        ...
        try:
            ...
            try:
                risk_msg = await risk_control_detected(page, "xiaohongshu")
            except Exception:
                risk_msg = None
            if risk_msg:
                logger.warning(f"[Xiaohongshu] Risk control detected: {risk_msg}")
