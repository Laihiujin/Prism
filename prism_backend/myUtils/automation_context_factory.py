...
def _read_platform_browser_choice_from_env(platform: str) -> Any:
    env_name = f"PRISM_PLATFORM_BROWSER_{platform.upper()}"
...
def _load_platform_browser_preferences() -> Dict[str, str]:
    ...
    env_payload = os.getenv("PRISM_PLATFORM_BROWSER_PREFERENCES")
    if env_payload:
        try:
            raw_preferences.update(json.loads(env_payload))
        except Exception as exc:
            logger.warning(f"[playwright] Failed to parse PRISM_PLATFORM_BROWSER_PREFERENCES: {exc}")

    settings_path = os.getenv("PRISM_RUNTIME_SETTINGS_PATH")
    if settings_path:
        try:
...
def _resolve_local_chrome_path(headless: bool) -> str | None:
    ...
    # Project Chromium is optional in development. Reuse the user's installed
    # Chrome and pass its explicit executable path to Patchright/Playwright.
    try:
        from utils.chrome_detector import get_chrome_executable

        system_chrome = Path(get_chrome_executable())
        if system_chrome.is_file():
            return str(system_chrome)
