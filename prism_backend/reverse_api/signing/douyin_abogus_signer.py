...
class DouyinABogusSigner:
    ...
    async def sign_request(
        self, query: str, body: str, user_agent: str, headers: dict[str, str] | None = None,
        *, url: str | None = None, method: str = "POST",
    ) -> dict[str, str]:
        """Return the current web query signature and D-Trait request header."""
        if not user_agent:
            raise ValueError("user_agent is required")
        request = json.dumps(
            {"query": query, "body": body, "userAgent": user_agent, "headers": headers or {},
        ...
        dtrait = response.get("x_tt_session_dtrait")
        if (
            not response.get("ok")
            or not isinstance(signature, str)
            or len(signature) < 100
            or not isinstance(dtrait, str)
            or not dtrait.startswith("d0_")
        ):
            raise SignerError(str(response.get("error") or "invalid a_bogus output"))
        result = {"a_bogus": signature, "x-tt-session-dtrait": dtrait}
        if isinstance(response.get("ms_token"), str) and response["ms_token"]:
            result["msToken"] = response["ms_token"]
        if isinstance(response.get("account_sdk_source_info"), str):
            result["account_sdk_source_info"] = response["account_sdk_source_info"]
        if isinstance(response.get("verify_fp"), str):
            result["verify_fp"] = response["verify_fp"]
        return result

    async def sign(self, query: str, body: str, user_agent: str) -> str:
        """Backward-compatible helper returning only ``a_bogus``."""
        return (await self.sign_request(query, body, user_agent))["a_bogus"]
