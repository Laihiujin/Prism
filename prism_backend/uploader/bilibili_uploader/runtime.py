...
GITHUB_RELEASE_LATEST_PAGE = "https://github.com/biliup/biliup/releases/latest"
GITHUB_RELEASE_EXPANDED_ASSETS = "https://github.com/biliup/biliup/releases/expanded_assets/{tag_name}"


def get_biliup_runtime_root() -> Path:
    return Path.home() / ".prism" / "tools" / "biliup"
...
def fetch_latest_release_from_page() -> dict:
    """Resolve the official release asset without consuming GitHub API quota."""
    headers = {"User-Agent": "Prism"}
    latest_page = requests.get(GITHUB_RELEASE_LATEST_PAGE, headers=headers, timeout=30)
    latest_page.raise_for_status()
    tag_name = latest_page.url.rstrip("/").rsplit("/", 1)[-1]
    if not tag_name or tag_name == "latest":
        raise RuntimeError("Could not resolve the latest biliup release tag")

    assets_page = requests.get(
        GITHUB_RELEASE_EXPANDED_ASSETS.format(tag_name=tag_name),
        headers=headers,
        timeout=30,
    )
    assets_page.raise_for_status()
    assets = [
        {
            "name": href.rsplit("/", 1)[-1],
            "browser_download_url": urljoin("https://github.com", href),
        }
        for href in re.findall(r'href="([^"]+)"', assets_page.text)
        if "/biliup/biliup/releases/download/" in href
    ]
...
def ensure_biliup_binary(force_check: bool = True) -> Path:
    ...
    try:
        ...
        try:
            release = fetch_latest_release_from_page()
        except Exception:
