---
name: prism-publish-youtube
description: "Prism YouTube发布：登录、校验、视频发布（及无图文）与 CLI/API/MCP 工具契约。"
version: 1.0.0
author: Prism
license: MIT
...
在 YouTube 上发布内容。能力已注册为独立工具，三层自动暴露：**MCP tool**、
**API**（`POST /api/v1/tool-catalog/<name>`）、**CLI**（`prism tool invoke <name>` / `prism youtube ...`）。

## 工具面

| 能力 | 工具名 |
|---|---|
| 视频发布 | `publish_video_to_youtube` |
| 图文/笔记发布 | `（YouTube 不支持图文/笔记发布）` |
| 登录 | `login_to_youtube` |
| 校验登录态 | `check_account_youtube` |

## 视频发布参数（`publish_video_to_youtube`）
...
OAuth/浏览器登录；有头，二维码展示给用户。

## 话题与坑

- 图文/笔记不支持。
- 一次输入完整 `#关键词` 再按空格确认/分隔，避免重复 `##`、换行或把多个话题粘成 `#话题1#话题2`。
- 账号 cookie 文件按 `runtime_home()/cookiesFile/youtube_<account>.json`。
