---
name: prism-publish-baijiahao
description: "Prism 百家号发布：登录、校验、视频发布（及无图文）与 CLI/API/MCP 工具契约。"
version: 1.0.0
author: Prism
license: MIT
...
在 百家号 上发布内容。能力已注册为独立工具，三层自动暴露：**MCP tool**、
**API**（`POST /api/v1/tool-catalog/<name>`）、**CLI**（`prism tool invoke <name>` / `prism baijiahao ...`）。

## 工具面

| 能力 | 工具名 |
|---|---|
| 视频发布 | `publish_video_to_baijiahao` |
| 图文/笔记发布 | `（百家号 不支持图文/笔记发布）` |
| 登录 | `login_to_baijiahao` |
| 校验登录态 | `check_account_baijiahao` |

## 视频发布参数（`publish_video_to_baijiahao`）
...
- 账号 cookie 文件按 `runtime_home()/cookiesFile/baijiahao_<account>.json`。
